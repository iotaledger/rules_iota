...
struct st_client_plugin_int
    *plugin_list[MYSQL_CLIENT_MAX_PLUGINS + MARIADB_CLIENT_MAX_PLUGINS];
#ifdef THREAD
...
static const char *check_plugin_version(struct st_mysql_client_plugin *plugin,
...
add_plugin(MYSQL *mysql, struct st_mysql_client_plugin *plugin, void *dlhandle,
  ...
  p = (struct st_client_plugin_int *)ma_memdup_root(
      &mem_root, (char *)&plugin_int, sizeof(plugin_int));
...
struct st_mysql_client_plugin *STDCALL mysql_client_register_plugin(
...
/* see <mysql/client_plugin.h> for a full description */
struct st_mysql_client_plugin *STDCALL mysql_load_plugin_v(MYSQL *mysql,
                                                           const char *name,
                                                           int type, int argc,
                                                           va_list args) {
  const char *errmsg;
  ...
  snprintf(dlpath, sizeof(dlpath) - 1, "%s/%s%s",
           mysql->options.extension && mysql->options.extension->plugin_dir
               ...
               : (env_plugin_dir) ? env_plugin_dir : MARIADB_PLUGINDIR,
           name, SO_EXT);
  ...
  if (!(dlhandle = dlopen((const char *)dlpath, RTLD_NOW))) {
    ...
    FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(),
                  MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), winmsg, 255, NULL);
    len = strlen(winmsg);
...
/* see <mysql/client_plugin.h> for a full description */
struct st_mysql_client_plugin *STDCALL mysql_load_plugin(MYSQL *mysql,
                                                         const char *name,
                                                         int type, int argc,
                                                         ...) {
